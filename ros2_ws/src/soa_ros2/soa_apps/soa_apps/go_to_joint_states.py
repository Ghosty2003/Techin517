...
from soa_interfaces.action import MoveToJointStates, Gripper
...
class GoToJointStates(Node):
   ...
   def send_joint_goal(self, joint_positions: list, joint_names: list, label: str = '') -> bool:
       ...
       )# TODO: send the goal asynchronously
       ...
       goal_handle = future.result()# TODO: get the result from future to see if the goal was accepted
   ...
   def _joint_feedback_callback(self, feedback_msg):
       ...
       self.get_logger().info(
           f'Joint feedback - max error: {feedback_msg.feedback.max_joint_error:.4f} rad'
       )

   def _gripper_feedback_callback(self, feedback_msg):
       # TODO: log the feedback from the gripper action
       self.get_logger().info(
           f'Gripper feedback: {feedback_msg.feedback}'
       )
