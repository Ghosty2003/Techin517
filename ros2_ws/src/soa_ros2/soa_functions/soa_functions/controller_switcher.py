...
from controller_manager_msgs.srv import SwitchController as CmSwitchController
...
class ControllerSwitcherNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('cm_service', '/controller_manager/switch_controller')
        # Comma-separated controller names for each mode.
        # Defaults match single-arm behaviour — override for bimanual arms.
        self.declare_parameter('jtc_controllers', 'arm_controller,gripper_controller')
        self.declare_parameter('fcc_controllers', 'arm_fwd_controller,gripper_fwd_controller')

        self._mode = self.get_parameter('initial_mode').value
        cm_service  = self.get_parameter('cm_service').value

        def _parse(param: str) -> list:
            return [c.strip() for c in self.get_parameter(param).value.split(',') if c.strip()]

        self.VALID = {
            'jtc':     _parse('jtc_controllers'),
            'forward': _parse('fcc_controllers'),
        }
